...
- **Submit via `fetch()`**, not a native navigation: `method="POST"`, prevent the default submit, show sending → success/error states, reset the form on success. Because the POST goes to an external endpoint via `fetch`, the page does **not** need `export const prerender = false`.
